...
check_chipset() {
    ...
    # =================== < CONFIG > ===================
    readonly local CHIPSET_LIST="scripts/devices.xml" # chipset file list
    ...
    while true;do
        ...
        if [ "$SUPPORT_MO" == false ] || [ "$SUPPORT_MO" == true ]; then break; fi

        cout=$(echo $(($cout+1)))
        if [ $cout -gt $length ] || [ "$cout" -eq $(echo $(($i+10))) ];then echo -e "\033[31mChipset not in list\033[0m";break ;fi # Catch out of range
    done
